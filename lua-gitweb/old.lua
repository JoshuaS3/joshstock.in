...
ngx.print("<pre class=\"container\"><code>")
...
ngx.print("<p>")
ngx.print("<a href=\"/",name,"/refs\">Refs</a> | ")
ngx.print("<a href=\"/",name,"/log/",headname.name,"\">Commit Log</a> | ")
ngx.print("<a href=\"/",name,"/tree/",headname.name,"\">Files</a> | ")
ngx.print("<a href=\"/",name,"/blob/",headname.name,"/README.md\">README</a> | ")
ngx.print("<a href=\"/",name,"/blob/",headname.name,"/LICENSE\">LICENSE</a>")
ngx.print("</p></div>")
...
for i,commit in pairs(commits_head) do
    ...
    ngx.print("<td class=\"subject\">",commit.subject:gsub("<","&lt;"),"</td>")
...
local iconfolder = [[<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="#FFE9A2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>]]
local iconfile = [[<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="#ffffff" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path><polyline points="13 2 13 9 20 9"></polyline></svg>]]

for i,v in pairs(files.dirs) do
    ngx.print("<tr>")
    ngx.print("<td>",iconfolder,"<a href=\"/",name,"/tree/master/",v,"\">",v,"</a></td>")
...
for i,v in pairs(files.files) do
    ngx.print("<tr>")
    ngx.print("<td>",iconfile,"<a href=\"/",name,"/blob/master/",v,"\">",v,"</a></td>")
    local lastedit = git.log(repo, "@ -1", v, 1, 0, false)[1]
    ngx.print("<td>",lastedit.subject,"</td>")
    ngx.print("<td class=\"timestamp\">",utils.iso8601(lastedit.timestamp),"</td>")
    ngx.print("<td class=\"hash\"><a href=\"/",name,"/commit/",lastedit.hash,"\">",lastedit.shorthash,"</a></td>")
